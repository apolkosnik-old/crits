...
def base_context(request):
    ...
    if git_repo_url:
        base_context['git_repo_link'] = "<a href='%s/commit/%s'>%s:%s</a>" % (str(git_repo_url), str(git_hash_long), str(git_branch), str(git_hash))
    else:
...
def user_preference_update(request, section):
    ...
    if request.method == 'POST' and request.is_ajax():
        ...
        if not pref or 'formclass' not in pref or not isinstance(pref['formclass'], collections.Callable):
